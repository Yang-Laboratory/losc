...
struct LocalizationTest:public ::testing::TestWithParam<string> {
    ...
    {
        S_ao.push_back(std::make_shared<Matrix> (nbasis, nbasis));
        for (size_t is = 0; is < 2; is++) {
            C_lo_basis.push_back(std::make_shared<Matrix> (nlo, nbasis));
            C_lo_ref.push_back(std::make_shared<Matrix> (nlo, nbasis));
            H_ao.push_back(std::make_shared<Matrix> (nbasis, nbasis));
        }
        for (size_t xyz = 0; xyz < 3; xyz++) {
            D_ao.push_back(std::make_shared<Matrix> (nbasis, nbasis));
        }
...
TEST_P(LocalizationTest, H2)
{
    ...
    string lo_basis_path = dir_path + "/./data/" + mol + "/lo_basis.bin";
    string lo_ref_path = dir_path + "/./data/" + mol + "/lo.bin";
    string H_ao_path = dir_path + "/./data/" + mol + "/dfa_h.bin";
    string D_ao_path = dir_path + "/./data/" + mol + "/dipole_ao.bin";
    string S_ao_path = dir_path + "/./data/" + mol + "/ao_overlap.bin";
    ...
    matrix::read_matrices_from_binary(C_lo_basis, lo_basis_path.c_str());
    matrix::read_matrices_from_binary(C_lo_ref, lo_ref_path.c_str());
    matrix::read_matrices_from_binary(H_ao, H_ao_path.c_str());
    matrix::read_matrices_from_binary(D_ao, D_ao_path.c_str());
    matrix::read_matrices_from_binary(S_ao, S_ao_path.c_str());
